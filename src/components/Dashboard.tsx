        ...
        <div className="fixed inset-0 z-[100] pointer-events-none flex flex-col">
            ...
            <header className={`backdrop-blur-3xl border-b px-4 md:px-12 py-4 flex items-center justify-between gap-4 pointer-events-auto transition-all duration-1000 relative z-[110] ${headerStyle}`}>
                    ...
                    <div className="flex flex-col shrink-0">
                        <div className="flex items-center gap-2 mb-1">
                           ...
                           {isLocationAuthorized && (
                               <div className="hidden sm:flex items-center gap-1 bg-emerald-500/10 border border-emerald-500/20 px-1.5 py-0.5 rounded text-[7px] font-black text-emerald-400">
                                  <div className="w-1 h-1 bg-emerald-400 rounded-full animate-pulse" /> SYNC
                               </div>
                           )}
                        </div>
                        <div className="flex items-center gap-3">
                            <div className="flex items-center -space-x-1.5">
                                <div className="w-5 h-5 rounded-full bg-blue-600 border-2 border-blue-400 shadow-lg z-10" />
                                <div className="w-5 h-5 rounded-full bg-yellow-500 border-2 border-yellow-300 shadow-lg" />
                            </div>
                            <span className="text-[10px] font-black tracking-widest text-white/40 uppercase">MI <span className="text-white/10 mx-1">/</span> CSK</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 flex justify-center min-w-0">
                    {matchData && (
                        <div className="flex items-center gap-6 bg-white/[0.03] backdrop-blur-3xl px-8 py-2.5 rounded-full border border-white/10 shadow-xl group">
                            <div className="flex items-center gap-3">
                                <span className="text-xs font-black text-blue-400">MI</span>
                                <span className="text-xl font-black tabular-nums tracking-tighter text-white">{matchData.homeScore}</span>
                            </div>
                            <div className="hidden sm:flex flex-col items-center border-x border-white/5 px-6">
                                <span className="text-xl font-black tabular-nums text-[var(--accent)] tracking-tighter glow-text">{matchClock}</span>
                            </div>
                            <div className="flex items-center gap-3">
                                <span className="text-xl font-black tabular-nums tracking-tighter text-white">{matchData.awayScore}</span>
                                <span className="text-xs font-black text-yellow-400">CSK</span>
                            </div>
                        </div>
                    )}
                </div>
                ...
                <div className="flex items-center justify-end gap-6 min-w-[300px]">
            ...
            {/* Mobile Navigation */}
            <nav className="lg:hidden bg-[#0A0A0B]/95 backdrop-blur-3xl border-t border-white/10 px-8 py-6 flex items-center justify-around pointer-events-auto safe-bottom">
                {navTabs.map((tab) => (
                    <button
                        key={tab.id}
                        onClick={() => setActiveTab(tab.id as any)}
                        className={`flex flex-col items-center gap-2 transition-all ${activeTab === tab.id ? 'text-[var(--accent)]' : 'text-white/20'}`}
                    >
                        <div className={`p-3 rounded-2xl ${activeTab === tab.id ? 'bg-[var(--accent-10)] border border-[var(--accent-30)]' : 'bg-white/5'}`}>
                            {tab.icon}
                        </div>
                    </button>
                ))}
